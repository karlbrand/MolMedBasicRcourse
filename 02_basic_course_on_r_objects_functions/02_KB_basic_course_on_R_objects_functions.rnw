%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Program: 02_KB_basic_course_on_R_objects_functions.rnw                  %
...
An object that does something to the fruits and veggies, a \emph{frying pan!} Remember the function we defined?
<<echo=FALSE>>=
...
of fruit, a tray veggies, another sublist of items
<<>>=
...
`mode' (mutually exclusive classification of basic structure) and a `class' (property used to determine how functions operate with object). Pay attention to your modes and classes.
...
\item R has an extensive set of built-in {\bf \em functions}, a few of which are listed below:
...
\item The two types of argument specification (positional and named argument
