...
\item A vector of letters called {\tt lettervec}, of class {\tt
    character}, from `a' to `d', using the function {\tt c()}.  
...
\item A vector (using {\tt c()} then {\tt factor()}) of class {\tt factor} called {\tt pets}, that looks approximately like this when printed:
  \begin{tabbing}
