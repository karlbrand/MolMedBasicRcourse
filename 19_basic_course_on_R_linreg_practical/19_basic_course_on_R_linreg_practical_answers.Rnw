%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Program: 19_basic_course_on_R_linreg_practical_answers.Rnw              %
...
lmbh
@
{\tt vitaminB12} is statistically significant ($p$-value less than 0.05). It explains about 9.3 percent of the variability in {\tt loghc}. The model with the estimates is
\begin{center}
...
Yes, 15.1 is in the range of the unlogged {\tt homocysteine} values: (10.9, 30.6).
 ...
 \newpage \item Now let's consider a framework where we want to use more than one predictor. We want to build a regression model for {\tt SAM} using {\tt vitaminB12}, {\tt cholesterol}, {\tt homocysteine} and {\tt folicacid\_erys} (folic acid red blood cells). Answer the following questions: 
  \begin{enumerate}
...
mlr
@
Only {\tt cholesterol} is statistically significant ($p$-value less than 0.05). The 4 variables explain 7.27 percent of the variability in {\tt SAM}. The model with the estimates is 
\begin{center}
