...
\author{Karl Brand\footnote{karl.brand@bayer.com}\hspace{1mm} and Elizabeth Ribble\footnote{emcclel3@msudenver.edu}}
\date{14-18 May 2018}
...
part of the course we'll learn how to use {\tt R} for it's initial
intended function: doing statistics quickly and effectively.
...
I attribute much of {\tt R}'s success to its GPL v2 license: created
\underline{by users for users} to run well on Linux, Apple and
Windows. These licenses give everyone the freedom to use, the freedom
to distribute and the freedom to contribute their own intellectual
input to the project. The result is that performance and fitness of
purpose is and always will be the primary focus. I attribute {\tt
  R}'s limited general use to its steep learning curve for
non-programmers. Base {\tt R} is also yet to take advantage of the
...
\begin{figure}
  ...
  \end{center}
  \caption{Get R: \url{http://www.r-project.org/}\url{http://www.illumina.com/technology/beadarray_technology.ilmn}}
\end{figure}

For Linux: \url{http://cran.r-project.org/bin/linux/}
  
For Mac: \url{http://cran.r-project.org/bin/macosx/}

For Windows: \url{http://cran.r-project.org/bin/windows/base/}
...
RStudio: \url{http://www.rstudio.com/ide/download/}

EMACS/ESS: \url{http://ess.r-project.org/index.php?Section=download/}

Here's a full list available editors:
\url{https://en.wikipedia.org/wiki/R_(programming_language)#Graphical_user_interfaces}

\subsection{Not convinced?}
\href{http://r4stats.com/articles/popularity/}{r4stats.com} has a
nice evaluation of {\tt R}'s value `in the field'.
More and more people are discussing R:\\
\includegraphics[width=5.5in]{fig_1a_listserv11.png}\\
\url{http://r4stats.files.wordpress.com/2012/04/fig_1a_listserv11.png}\\

%\includegraphics[width=4in]{fig_3_rexersurvey.png}\\
%\url{http://r4stats.files.wordpress.com/2012/04/fig_3_rexersurvey.png}\\

\includegraphics[width=4in]{fig_4_kdnuggets1.png}\\
{\tt R} is the most used software for getting data mining and big data ``done''.
\url{http://r4stats.files.wordpress.com/2012/04/fig_4_kdnuggets1.png}\\
...
\includegraphics[width=5in]{fig_8_cran.png}\\
There's an enormous number of packages offering custom functionality.
\url{http://r4stats.files.wordpress.com/2012/04/fig_8_cran.png}

\includegraphics[width=4.3in]{bioconductor.png}\\ Bioconductor,
initiated in 2001 for the analysis of genomics data, has grown from 15
to (currently) 1024 software packages for omics and life science
analyses.  \url{http://www.bioconductor.org/}

\includegraphics[width=4.3in]{companies.png}\\
Used internally by lots of big, profitable companies. \url{http://www.revolutionsanalytics.com/what-is-open-source-r/companies-using-r.php}
...
\href{http://www.forbes.com/sites/smcnally/2010/11/10/names-you-need-to-know-in-2011-r-data-analysis-software/}{Link to reference}
\newpage Many other statistical and analytical programs have implemented
...
is first and foremost a statistics focused language. Despite this,
it's close to being in the top 10 of the most popular computing
languages.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
\newpage\section{What \emph{is} {\tt R} and how do we use it}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Definition(s)}

\begin{itemize}
\item \textbf{Wikipedia}: `R is a free software programming language and
  software environment for statistical computing and graphics... is an
  implementation of the S programming language combined with lexical
  scoping semantics inspired by Scheme.'
  \url{http://en.wikipedia.org/wiki/R_(programming_language)}

\item  \textbf{Comprehensive {\tt R} Archive Network (CRAN):} `R is ``GNU S'', a
  freely available language and environment for statistical computing
  and graphics which provides a wide variety of statistical and
  graphical techniques: linear and nonlinear modelling, statistical
  tests, time series analysis, classification, clustering, etc.'
  \url{http://cran.xl-mirror.nl/}

\item  \textbf{My definition:} A command based, object oriented, functional
...
KISS, i.e., keep it simple stupid. 
  \begin{itemize}
    ...
    \item Since the commands are just text, it's easy to write a series of commands using any program built for working with text, i.e., text editors and word
    ...
    \item The easiest, most efficient way to write
    {\tt R} code is using a text editor or \emph{integrated development
...
RStudio on your PC and try it now.
...
A few other things about RStudio:
  ...
  \begin{itemize}
    \item move the focus between panes using the key board shortcuts {\tt
      ...
      respectively
    \item little asterisk next your source code file name indicates
      unsaved code
    \item \textbf{save your source code!}
  \end{itemize}
  
%% \noindent Typically we import or `read' data into {\tt R}.
%% <<eval=FALSE>>=
%% read.table(file = "a_separated_ saved_excel.csv",
%%            sep = ",", header = TRUE, stringsAsFactors = FALSE)
%% @

%% Note the argument {\tt stringsAsFactors = FALSE} and also see the
%% function {\tt read.csv()}. For a tab delimited text {\tt .txt} file
%% use: \begin{verbatim} sep = "\t" \end{verbatim}
...
\url{http://cran.r-project.org/doc/contrib/Lemon-kickstart/kr_scrpt.html}:
 ...
 almost as much strife as the question of exactly what the <Delete>
 key should do. The convention that the slash (/) character is used as
 a separator in the notation for a filesystem path is not
 universal. PC-DOS, for example, used the backslash (\verb|\|). Like Fords vs
 Chevrolets, French vs English, and whether you crack the egg at the
 big or small end, it doesn't really matter a rat's behind which one
 you use as long as it works. To make it work in R, however, where the
 *NIX convention of using the backslash as an escape character is
 respected, you will have to double backslashes in paths for them to
 be read properly. Thus if the filename above was:
...
\begin{itemize}
\item Clean your environment and start again to be sure your messy environment
  isn't the cause of your issue. In RStudio, click the broom in the Environment tab; in any R session you can also run:
<<eval=FALSE>>=
# not run:
rm(list=ls())                 
ls()
