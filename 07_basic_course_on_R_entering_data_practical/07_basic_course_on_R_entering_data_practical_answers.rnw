...
\subsection{Use R to do the following exercises on ``mouse'' data.} 
...
@
  ...
  \item Join {\tt color} and {\tt weight} together in a data structure with 2 columns and 8 rows, and assign it to the object {\tt mice}. 
{\color{red} 
<<>>=
mice <- data.frame(color = color, weight = weight)
@
}
  \item What is the data structure of {\tt mice}? What are the dimensions? 
{\color{red} 
<<>>=
mice
str(mice)
dim(mice)
@
Using {\tt str} we see that {\tt mice} is a data frame with dimensions 8 x 2, with variable {\tt color} a factor with 5 levels and {\tt weight} a numeric vector. The function {\tt dim} can also be used to extract only the dimensions. 
}
  \item Display only the 3rd row of {\tt mice}.
{\color{red}
<<>>=
mice[3, ]
@
}
  \item Display only the 2nd column (``weight'') of {\tt mice}. Do so in two different ways.
...
mice[, "weight"] 
@
\noindent and if {\tt mice} is a data frame:
<<eval=FALSE>>=
mice[[2]] 
mice$weight 
mice["weight"] 
...
\item If the structure of {\tt mice} is not a data frame, turn it into one and call it {\tt micedf}. If {\tt mice} is already a data frame, go ahead and rename it {\tt micedf} anyway. Make sure that the weights are not factors!

{\color{red}
<<>>=
micedf <- mice
@
\noindent If you created a matrix, e.g. 
<<>>=
mice <- cbind(color, weight)
@
\noindent then do the following:
<<>>=
...
\noindent A list with three items with the given names preceded by
  ...
  \item View {\tt mylist}. What does it look like (expand the window
    if necessary)?  
    {\color{red}
<<>>=
View(mylist)
@ 
A matrix with each column representing a column from the columns of
the two data frames, plus a final column where the two entries from
the third element are repeated to fill up the rows of the matrix. Hence, {\tt View} is not extremely useful for looking at lists.}
