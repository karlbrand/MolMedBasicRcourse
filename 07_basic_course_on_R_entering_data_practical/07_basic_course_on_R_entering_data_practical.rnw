...
\begin{enumerate}[{1.1}.1]
  ...
  \item Create a list containing three elements and assign it to {\tt mylist}: 
    \begin{itemize}
      ...
      (Did you get an error? Make sure that the second column is numeric and if it isn't, change it!)
  ...
  \vspace{12 mm}
  \item View {\tt mylist}. What does it look like? What does it look like (expand the window if necessary)?
  \vspace{12 mm}
  \item Display only the second element of {\tt mylist}. Do so in two different ways. 
  \vspace{5 mm}
