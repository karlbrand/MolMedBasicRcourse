%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Program: 15_basic_course_on_R_distfreeANOVA_practical.Rnw               %
...
% @
% 
% Looks like the mean concentration for cyclopropane is higher than the other 2.
% }
...
\item What do you conclude about the effect of anesthetics on plasma epinephrine concentration (use a significance level of 0.05)?\\
    \vspace{50mm}
% {\color{red}
% Since the $p$-value (0.2725) is greater than 0.05, the data do not provide sufficient evidence to conclude the 3 anesthetics have differing effects on the plasma epinephrine concentration.
% }
  \end{enumerate}
...
\item Answer the following questions using the mom data:
    \begin{enumerate}
      \item Test at significance level 0.05 for equality of the three population centers (use the Kruskal-Wallis test).
  \vspace{50mm}
% {\color{red}
% <<>>=
...
% @
% 
% With a $p$-value 0.009894, which is less than 0.05, we are able to reject the null that all the group means are equal and therefore conclude at least two of the mean cortisol levels of the three groups are different. 
% }      
